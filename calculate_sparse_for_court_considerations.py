...
path_str = './data/processed/_sparse_court'
Path(path_str).mkdir(parents=True, exist_ok=True)

def __gen_lexical_weights(model, texts):
    l = model.encode(texts, 
                     batch_size=10, 
                     return_dense=False, 
                     return_sparse=True, 
...
for rows in tqdm(chunked_list, total=len(chunked_list)):
    if os.path.exists(os.path.join(path_str, f"{file_no}.pkl")):
        file_no += 1
        continue
    sparse_l = batch_calc_sparse(rows)
